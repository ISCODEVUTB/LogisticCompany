...
from ..repository.shipping_order_repo import (cancel_order, get_all_orders,
                                                get_order_by_id,
                                                get_order_by_tracking_code,
                                                save_order)
from ..repository.shipping_order_repo import \
...
from .tracking_service_client import send_tracking_event
...
async def create_shipping_order(
    ...
    # Call send_tracking_event, its result (now a dict) must not be returned
    # by this function
...
async def update_shipping_order_status(order_id: str, new_status: str) -> bool:
    ...
    order_data = repo_update_order_status(
        order_id, new_status
    )  # This is from shipping_order_repo.py

    if order_data is not None:  # Explicitly check for None
        logging.info(
            f"Order ID: {order_id} found and updated in repo. Order data: {order_data}"
        )
        try:
            tracking_code = order_data["tracking_code"]
            ...
            if not updated_at_iso:
                logging.warning(
                    f"updated_at not found for order {order_id} after update, using current time for tracking."
                )
                updated_at_dt = datetime.now(
                    timezone.utc
                )  # Fallback, though repo should set it
            else:
                # Handle potential 'Z' if FastAPI/Pydantic doesn't strip it for
                # fromisoformat
                if isinstance(updated_at_iso, str) and updated_at_iso.endswith("Z"):
                ...
                ):  # If it's already a datetime obj
                    ...
                    updated_at_dt = updated_at_dt.astimezone(
                        timezone.utc
                    )  # Normalize to UTC
            ...
            logging.info(
                f"Sending tracking event for order ID: {order_id}, tracking_code: {tracking_code}, status: {new_status}"
            )
            ...
            logging.error(
                f"Error sending tracking event for order ID: {order_id} after status update: {e}",
                exc_info=True,
            )
            # Decide if this should still return True as the order status was updated.
            # For now, let's say the primary operation (status update)
            # succeeded.
            return True
    else:
