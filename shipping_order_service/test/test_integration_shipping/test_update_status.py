...
async def test_update_shipping_order_status(mocker):
    ...
    mock_tracking_dict_response = {
        # Essential for response.json()["order_id"]
    ...
    # If the app's status update logic also calls this, it will also receive
    # this response.
    mocker.patch(
    ...
    async def mock_post_side_effect(*args, **kwargs):
        ...
        if (
            "/status" in str(args)
            or "/status" in str(kwargs)
            or "status" in str(kwargs.get("params", {}))
        ):
    ...
    async with AsyncClient(transport=transport, base_url="https://test") as client:
        ...
        status_response = await client.post(
            f"/api/orders/{order_id}/status", params={"status": "shipped"}
        )
