...
async def test_get_shipping_order_by_id(mocker):
    ...
    mock_tracking_dict_response = {
        # Essential for create_response.json()["order_id"]
    ...
    mock_tracking_response = Response(
        201,
        json={
            "order_id": "mocked-order-123",
            "tracking_code": "mocked-tracking-123",
    ...
    # Mock for GET operation - needs to return 200 status code with order
    # details
    ...
    # Mock GET requests
    mocker.patch(
        "shipping_order_service.app.services.tracking_service_client.httpx.AsyncClient.get",
        new_callable=mocker.AsyncMock,
        return_value=mock_get_response,
    )

    # Note: The test logic for GET /api/orders/{order_id} seems to rely on the
