...
from app.schemas.tracking_schemas import TrackingEventCreate, TrackingEventOut, TrackingHistoryOut
...
def test_add_tracking_event(service):
    ...
    with patch("app.services.tracking_service.save_tracking_event") as mock_save:
...
def test_get_history(service):
    ...
    with patch("app.services.tracking_service.get_tracking_events_by_code", return_value=[mock_event]):
