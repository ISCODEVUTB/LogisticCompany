...
# Patch save_driver where it's looked up in the app.services.driver_service module
@patch('app.services.driver_service.save_driver') 
def test_create_driver_success(mock_save_driver): 
    # mock_load_drivers is removed as it's not directly used by create_driver service function
    ...
    # Note: Pydantic v1 models might not automatically set timezone to UTC unless explicitly done in model
    # For this test, we'll assume it's set or not strictly check tzinfo if not critical for this unit.
    # If Driver model's default_factory for created_at/updated_at uses timezone.utc, this is fine.
    ...
    # To check arguments: mock_save_driver.assert_called_once_with(driver_instance_arg)
    # where driver_instance_arg is the instance of Driver passed to save_driver.
    # For simplicity here, just checking it was called. Can be more specific if needed.
