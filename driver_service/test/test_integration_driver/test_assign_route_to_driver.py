...
async def test_assign_route_success():
    ...
    async with AsyncClient(transport=transport, base_url="https://test") as client:
        ...
        # Asignar ruta
        response_assign = await client.patch(
            f"/api/drivers/{driver_id}/assign-route?route_id=R123"
        )
        assert response_assign.status_code == 200
        assert (
            response_assign.json()["message"]
            == f"Route R123 assigned to driver {driver_id}"
        )
